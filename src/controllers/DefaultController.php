...
class DefaultController extends BaseController
{
    ...
    public function actionDeletepost($cid = null, $fid = null, $tid = null, $pid = null)
    {
        ...
        if ($post->thread->locked == 1 
                && !User::can(Rbac::PERM_UPDATE_THREAD, ['item' => $post->thread])) {
        ...
        if (!User::can(Rbac::PERM_DELETE_OWN_POST, ['post' => $post]) 
                && !User::can(Rbac::PERM_DELETE_POST, ['item' => $post])) {
            $this->error(
                Yii::t(
                    'podium/flash', 
                    'Sorry! You do not have the required permission to perform this action.'
    ...
    public function actionEdit($cid = null, $fid = null, $tid = null, $pid = null)
    {
        ...
        if (!User::can(Rbac::PERM_UPDATE_OWN_POST, ['post' => $post]) 
                && !User::can(Rbac::PERM_UPDATE_POST, ['item' => $post])) {
            $this->error(
                Yii::t(
                    'podium/flash', 
                    'Sorry! You do not have the required permission to perform this action.'
    ...
    public function actionForum($cid = null, $id = null, $slug = null, $toggle = null)
    {
        ...
        if (in_array(strtolower($toggle), ['new', 'edit', 'hot', 'pin', 'lock', 'all'])) {
            if (strtolower($toggle) == 'all') {
                ...
                $filters[strtolower($toggle)] = empty($filters[strtolower($toggle)]) 
                                                    || $filters[strtolower($toggle)] == 0 
                                                        ? 1 : 0;
            }
        ...
        foreach ($categories->each() as $cat) {
            ...
            foreach ($forums->each() as $for) {
                if ($for->category_id == $cat->id) {
                    $catlist[$for->id] = (User::can(
                                                Rbac::PERM_UPDATE_THREAD, 
                                                ['item' => $for]
                                            ) ? '* ' : '') 
    ...
    public function actionPost($cid = null, $fid = null, $tid = null, $pid = null)
    {
        ...
        if ($thread->locked == 1 
                && !User::can(Rbac::PERM_UPDATE_THREAD, ['item' => $thread])) {
            $this->info(Yii::t('podium/flash', 'This thread is locked.'));
            return $this->redirect([
                'default/thread', 
                'cid'  => $thread->forum->category->id, 
        ...
        if (is_numeric($pid) && $pid > 0) {
            ...
            if ($replyFor) {
                $model->content = Helper::prepareQuote(
                                    $replyFor, 
                                    Yii::$app->request->post('quote')
                                );
            }
        }
