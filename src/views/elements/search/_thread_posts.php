...
?><td class="podium-thread-line">
    <a href="<?= Url::to(['default/show', 'id' => $model->post->id]) ?>" class="podium-go-to-new pull-right" style="margin-right:10px" data-toggle="popover" data-container="body" data-placement="left" data-trigger="hover focus" data-html="true" data-content="<small><?= Html::encode(strip_tags($model->post->content)) ?><br><strong><?= $model->post->user->getPodiumName() ?></strong> <?= Yii::$app->formatter->asRelativeTime($model->post->updated_at) ?></small>" title="<?= Yii::t('podium/view', 'Found Post') ?>">
...
<td>
    <small><?= $model->post->user->getPodiumTag() ?><br><?= Yii::$app->formatter->asDatetime($model->post->created_at) ?></small>
</td>