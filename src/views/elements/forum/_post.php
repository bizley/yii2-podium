...
$this->registerJs("$('.podium-thumb-up').click(function (e) { e.preventDefault(); var link = $(this); var parent = link.closest('.popover.podium'); link.removeClass('btn-success').addClass('disabled text-muted'); $.post('" . Url::to(['forum/thumb']) . "', {thumb:'up', post:$(this).data('post-id')}, null, 'json').fail(function(){ console.log('Thumb Up Error!'); }).done(function(data){ parent.find('.podium-thumb-info').html(data.msg); if (data.error == 0) { var cl = 'default'; if (data.summ > 0) cl = 'success'; else if (data.summ < 0) cl = 'danger'; parent.find('.podium-rating').removeClass('label-default label-danger label-success').addClass('label-' + cl).text(data.summ); parent.find('.podium-rating-details').text(data.likes + ' / ' + data.dislikes); } parent.find('.podium-thumb-down').removeClass('disabled text-muted').addClass('btn-danger'); }); });");
$this->registerJs("$('.podium-thumb-down').click(function (e) { e.preventDefault(); var link = $(this); var parent = link.closest('.popover.podium'); link.removeClass('btn-danger').addClass('disabled text-muted'); $.post('" . Url::to(['forum/thumb']) . "', {thumb:'down', post:$(this).data('post-id')}, null, 'json').fail(function(){ console.log('Thumb Down Error!'); }).done(function(data){ parent.find('.podium-thumb-info').html(data.msg); if (data.error == 0) { var cl = 'default'; if (data.summ > 0) cl = 'success'; else if (data.summ < 0) cl = 'danger'; parent.find('.podium-rating').removeClass('label-default label-danger label-success').addClass('label-' + cl).text(data.summ); parent.find('.podium-rating-details').text(data.likes + ' / ' + data.dislikes); } parent.find('.podium-thumb-up').removeClass('disabled text-muted').addClass('btn-success'); }); });");
...
<?php endif; ?>
                <ul class="podium-action-bar list-inline">
                    <?= Html::beginForm(['forum/post', 'cid' => $model->thread->category_id, 'fid' => $model->forum_id, 'tid' => $model->thread_id, 'pid' => $model->id], 'post', ['class' => 'quick-quote-form']); ?>
...
<?php else: ?>
                    <li><a href="#" class="btn btn-success btn-xs podium-thumb-up" data-post-id="<?= $model->id ?>" data-toggle="tooltip" data-placement="top" title="<?= Yii::t('podium/view', 'Thumb up') ?>"><span class="glyphicon glyphicon-thumbs-up"></span></a></li>
<?php endif; ?>
<?php if ($model->thumb && $model->thumb->thumb == -1): ?>
                    <li><a href="#" class="btn btn-xs disabled text-muted podium-thumb-down" data-post-id="<?= $model->id ?>" data-toggle="tooltip" data-placement="top" title="<?= Yii::t('podium/view', 'Thumb down') ?>"><span class="glyphicon glyphicon-thumbs-down"></span></a></li>
<?php else: ?>
                    <li><a href="#" class="btn btn-danger btn-xs podium-thumb-down" data-post-id="<?= $model->id ?>" data-toggle="tooltip" data-placement="top" title="<?= Yii::t('podium/view', 'Thumb down') ?>"><span class="glyphicon glyphicon-thumbs-down"></span></a></li>
<?php endif; ?>
                    <li><a href="<?= Url::to(['forum/report', 'cid' => $model->thread->category_id, 'fid' => $model->forum_id, 'tid' => $model->thread_id, 'pid' => $model->id]) ?>" class="btn btn-warning btn-xs" data-pjax="0" data-toggle="tooltip" data-placement="top" title="<?= Yii::t('podium/view', 'Report post') ?>"><span class="glyphicon glyphicon-flag"></span></a></li>
<?php endif; ?>
<?php if ($model->author_id == $loggedId || User::can(Rbac::PERM_DELETE_POST, ['item' => $model->thread])): ?>
                    <li><a href="<?= Url::to(['forum/deletepost', 'cid' => $model->thread->category_id, 'fid' => $model->forum_id, 'tid' => $model->thread_id, 'pid' => $model->id]) ?>" class="btn btn-danger btn-xs" data-pjax="0" data-toggle="tooltip" data-placement="top" title="<?= Yii::t('podium/view', 'Delete Post') ?>"><span class="glyphicon glyphicon-trash"></span></a></li>
<?php endif; ?>
                </ul>
            </div>
        </div>
