...
<?php if (!empty($model->latest) && !empty($model->latest->user)): ?>
    <small><?= $model->latest->user->user->getPodiumTag() ?><br><?= Yii::$app->formatter->asDatetime($model->latest->created_at) ?></small>
<?php endif; ?>
</td>